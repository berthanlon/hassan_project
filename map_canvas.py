...
def _to_screen(lat, lng, min_lat, max_lat, min_lng, max_lng, width, height, pad):
    ...
    x = width  // 2 if lng_range < 1e-9 else pad + int((lng - min_lng) / lng_range * usable_w)
    y = height // 2 if lat_range < 1e-9 else height - pad - int((lat - min_lat) / lat_range * usable_h)
...
def draw_route(canvas, depot, route, done_indices, current_index=None):
    ...
    Args:
        ...
        depot:         Depot Location (start/end).
        route:         Ordered list of stop Locations.
        done_indices:  Set of 0-based stop indices that are complete.
        current_index: Index of the current active stop (or None).
    """
    canvas.delete("all")
    ...
    def coords(loc):
        return _to_screen(loc.lat, loc.lng,
                          min_lat, max_lat, min_lng, max_lng, w, h, pad)

    # Background
    canvas.create_rectangle(0, 0, w, h, fill=COLOUR_BG, outline="")

    # Grid lines
    for i in range(5):
        gy = pad + (h - 2 * pad) * i // 4
        gx = pad + (w - 2 * pad) * i // 4
        canvas.create_line(pad, gy, w - pad, gy, fill=COLOUR_GRID, width=1)
        canvas.create_line(gx, pad, gx, h - pad, fill=COLOUR_GRID, width=1)

    # Full route lines (dashed, dim)
    ...
    # Completed segments (solid, bright)
    if done_indices:
        prev_loc = depot
    ...
    for i, stop in enumerate(route):
        ...
        # Outer ring for current stop (no transparency - just a plain ring)
        if is_next:
    ...
    # Legend
    canvas.create_oval(8, h - 50, 18, h - 40, fill=COLOUR_DEPOT,     outline="")
    canvas.create_text(22, h - 45, text="Depot", anchor="w",
                       fill=COLOUR_STOP_MUTED, font=("Courier", 8))
    canvas.create_oval(8, h - 35, 18, h - 25, fill=COLOUR_STOP_DONE, outline="")
    canvas.create_text(22, h - 30, text="Done",  anchor="w",
                       fill=COLOUR_STOP_MUTED, font=("Courier", 8))
    canvas.create_oval(8, h - 20, 18, h - 10, fill=COLOUR_STOP_NEXT, outline="")
    canvas.create_text(22, h - 15, text="Next",  anchor="w",
                       fill=COLOUR_STOP_MUTED, font=("Courier", 8))