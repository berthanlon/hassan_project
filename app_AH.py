...
class InputFrame(tk.Frame):
    ...
    def _on_samples_loaded(self, depot_data, stops_data):
        if depot_data:
            self.app.depot = algorithm_AH.Location(**depot_data)
            txt = "OK  " + depot_data["postcode"] + "  -  " + depot_data["district"]
            self.depot_status.configure(text=txt, fg=ACCENT)
        for r in stops_data:
            if r:
                self.app.stops.append(algorithm_AH.Location(**r))
        self._refresh_listbox()
        self._update_go_btn()
...
class OptimiseFrame(tk.Frame):
    ...
    def _run(self):
        ...
        result = algorithm_AH.optimise_route(
...
class DeliveryFrame(tk.Frame):
    ...
    def _build(self):
        ...
        for i, stop in enumerate(stops):
            ...
            if is_done:
                ...
                dist = round(algorithm_AH.haversine(prev_loc, stop), 2)
                tk.Label(info,
    ...
    def _tick(self, idx):
        ...
        dist_km     = algorithm_AH.haversine(prev_loc, stops[idx])
