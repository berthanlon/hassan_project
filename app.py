...
Four frames:
    InputFrame    - Add depot and stop postcodes
    OptimiseFrame - Shows algorithm progress
    DeliveryFrame - Live delivery tracking with tick-off
    ReportFrame   - Final statistics and report export
"""
...
def make_btn(parent, text, cmd, accent=True, width=None):
...
class InputFrame(tk.Frame):
    ...
    def _build(self):
        ...
        make_btn(depot_row, "Set", self._set_depot).pack(side="left", padx=(8, 0))
        ...
        make_btn(stop_row, "Add", self._add_stop).pack(side="left", padx=(8, 0))
        ...
        make_btn(stop_card, "Remove Selected", self._remove_stop,
        ...
        make_btn(sample_card, "Load Sample Data", self._load_samples,
...
class DeliveryFrame(tk.Frame):
    ...
    def _build(self):
        ...
        sb = tk.Scrollbar(outer, bg=SURFACE, troughcolor=BG)
        sb.grid(row=0, column=1, sticky="ns")

        sc = tk.Canvas(outer, bg=BG, highlightthickness=0,
                       yscrollcommand=sb.set)
        sc.grid(row=0, column=0, sticky="nsew")
        sb.configure(command=sc.yview)
        ...
        for i, stop in enumerate(stops):
            is_done = i in done
            is_cur  = (i == cur) and not is_done
            is_future = not is_done and not is_cur
            ...
            pc_colour = MUTED if is_done else (ACCENT if is_cur else TEXT)
            tk.Label(info, text=stop.postcode,
                     font=("Courier", 12, "bold"),
                     bg=card_bg, fg=pc_colour,
            ...
            # Status line
            if is_done:
                rec = next(r for r in self.app.delivery_records if r.stop_index == i)
                status = ("Delivered at " + rec.arrived_at.strftime("%H:%M")
                          + "   |   " + str(round(rec.distance_from_prev_km, 2)) + " km"
                          + "   |   " + str(round(rec.travel_minutes, 1)) + " min travel")
            ...
            if is_done:
                ...
                # Greyed out button for future stops (still clickable in case needed)
                tk.Button(right_side,
                          text="Deliver",
                          command=lambda idx=i: self._tick(idx),
                          bg=SURFACE2, fg=MUTED,
        ...
        if cur > 0 and cur < total:
            self.after(150, lambda: self._scroll_to_stop(sc, inner, cur))
...
class ReportFrame(tk.Frame):
    ...
    def _build(self):
        ...
        rep = report_module.DeliveryReport(
        ...
        make_label(hdr, "Delivery Report", FONT_TITLE, ACCENT).pack(side="left")

        btn_row = tk.Frame(hdr, bg=BG)
        btn_row.pack(side="right")
        make_btn(btn_row, "Save Report", self._save_report).pack(side="left", padx=(0, 8))
        make_btn(btn_row, "New Route", self._new_route, accent=False).pack(side="left")

        stats_f = tk.Frame(self, bg=BG)
        stats_f.pack(fill="x", pady=(0, 10))

        saved = result.initial_distance_km - result.total_distance_km
        pct   = saved / result.initial_distance_km * 100 \
                if result.initial_distance_km > 0 else 0

        stat_data = [
            (str(round(rep.total_minutes)),                   "TOTAL MINUTES"),
            (str(round(rep.total_km, 1)) + " km",            "KM TRAVELLED"),
            (str(len(records)),                                "STOPS DONE"),
            (str(round(rep.avg_minutes_per_stop, 1)),         "AVG MIN/STOP"),
            (str(round(rep.fastest_stop.travel_minutes, 1)), "FASTEST (MIN)"),
            (str(round(rep.slowest_stop.travel_minutes, 1)), "SLOWEST (MIN)"),
        ]
        for val, lbl in stat_data:
            ...
            tk.Label(c, text=lbl, font=("Courier", 7),
        ...
        for line in [
            ...
            "Saved:           " + str(round(saved, 2)) + " km (" + str(round(pct, 1)) + "%)",
        ...
        tbl_frame = tk.Frame(right, bg=BG)
        tbl_frame.pack(fill="both", expand=True)

        sb = tk.Scrollbar(tbl_frame, bg=SURFACE, troughcolor=BG)
        sb.pack(side="right", fill="y")
        ...
        sb.configure(command=tbl.yview)
        ...
        fast_idx = rep.fastest_stop.stop_index
        slow_idx = rep.slowest_stop.stop_index
        ...
        tbl.tag_configure("fast", foreground=GREEN)
        tbl.tag_configure("slow", foreground=RED)
